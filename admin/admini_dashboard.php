	  ...
	  <div>
		...
		<div class="pd-ltr-20">
			...
			<div class="card-box mb-30">
				<div class="pd-20">
				<!-- Start Pagination for "LATEST CUSTOMER SERVICED" -->
				<?php
            $limitService = 5; // Number of records to display per page for customer service
            $pageService = isset($_GET['pageService']) ? $_GET['pageService'] : 1; // Current page number for customer service

            $offsetService = ($pageService - 1) * $limitService; // Calculate the OFFSET for customer service

            $totalServiceRecordsQuery = "SELECT COUNT(*) as total FROM your_customer_service"; // Replace with your actual table name
            $totalServiceRecordsResult = mysqli_query($conn, $totalServiceRecordsQuery);
            $totalServiceRecordsRow = mysqli_fetch_assoc($totalServiceRecordsResult);
            $totalServiceRecords = $totalServiceRecordsRow['total'];

            $sqlService = "SELECT * FROM your_customer_service ORDER BY service_id DESC LIMIT $limitService OFFSET $offsetService"; // Replace with your actual table name
